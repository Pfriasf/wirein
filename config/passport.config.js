...
passport.use('local-auth', new LocalStrategy({
    ...
    User.findOne({ email: email })
        .then((user) => {
            if (!user) {
                next(null, false, { error: "The email address or password is incorrect" })
            } else {
                    ...
                    .then(match => {
                        if (match) {
                            if (user.active) {
                                ...
                                next(null, false, { error: "Check Your email to activate your account" })
                            }
                        } else {
                            next(null, false, { error: "The email address or password is incorrects" })
                        }
                    })
            }
        })
